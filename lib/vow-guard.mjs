...
export const findNgCategory = (raw) => {
  const t = String(raw || "").trim();
  if (!t) return "empty";
  if ([...t].length > VOW_MAX_LEN) return "long";

  const n = normalizeSafe(t);
  if (n.length < VOW_MIN_LEN) return "short";

  const personalHit = looksPersonal(t);
